...
async def trigger_manual_review(game_id: str):
    """
    Launches the LangGraph Visual Review Pipeline for a specific game proposal.
    Useful for manual QA testing of grounded research and visual correlation.
    """
    try:
        ...
        logger.info(f"Manual trigger received for: {game_title} ({game_id})")

        # 2. Run the pipeline (synchronously for the API response during QA testing)
        # Note: In production you might want this as a BackgroundTask
        final_state = await run_pipeline_with_tracking(game_id, game_title)
